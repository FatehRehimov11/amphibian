...
fun AmphibianApp(modifier: Modifier = Modifier){
    ...
    Scaffold(
        ...
        topBar = {AmphibianAppTopBar(scrollBehavior = scrollBehavior)}
    ) {
        HomeScreen(
            modifier = modifier.padding(it),
            uiState = amphibianViewModel.amphibianUiState,
            retryAction = amphibianViewModel::getAmphibianCards
        )
    }
}
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun AmphibianAppTopBar(scrollBehavior: TopAppBarScrollBehavior){
    TopAppBar(title = {
        Text("Amphibians")
    },
